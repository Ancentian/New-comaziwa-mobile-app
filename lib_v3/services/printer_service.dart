...
class PrinterService {
  ...
  static Future<bool> connectTest(BuildContext context) async {
    try {
      String? address = _printerAddress ?? AutoPrintService.getDefaultPrinterAddress();
      if (address == null) {
      ...
      if (_controller != null && address != null) {
        try {
          ...
          Fluttertoast.showToast(msg: 'Connection test sent', backgroundColor: Colors.green);
          return true;
        } catch (e) {
      ...
      Fluttertoast.showToast(msg: 'Printer not ready', backgroundColor: Colors.red);
      return false;
    } catch (e) {
      Fluttertoast.showToast(msg: 'Connection test error: $e', backgroundColor: Colors.red);
      return false;
    }
  }
  ...
  static Future<bool> printRawBytes(Uint8List bytes, BuildContext context) async {
    ...
    try {
      final address = _printerAddress ?? AutoPrintService.getDefaultPrinterAddress();
      if (address == null) {
        final ok = await selectPrinter(context);
        if (!ok) return false;
      }
      ...
      Fluttertoast.showToast(msg: 'printRawBytes error: $e', backgroundColor: Colors.red);
      return false;
    }
  }
  ...
  static Uint8List escNewLine([int n = 1]) => Uint8List.fromList(List.filled(n, 0x0A));
  ...
  static Future<bool> printEscPosMilk(Map<String, dynamic> data, BuildContext context) async {
    final lines = <String>[];
  ...
  static Future<bool> selectPrinter(BuildContext context) async {
    try {
      ...
      if (device == null) {
        Fluttertoast.showToast(msg: "No printer selected", backgroundColor: Colors.orange);
        return false;
      }
      _printerAddress = device.address;
      Fluttertoast.showToast(msg: "Printer selected: ${device.name}", backgroundColor: Colors.green);
      return true;
    } catch (e) {
      final errorMsg = e.toString().toLowerCase();
      
      // Check for common Bluetooth errors
      if (errorMsg.contains('bluetooth') && (errorMsg.contains('off') || errorMsg.contains('disabled'))) {
        showDialog(
          context: context,
          builder: (ctx) => AlertDialog(
            title: Row(
  ...
  static Future<bool> printReceiptWidget(Receipt receiptWidget, BuildContext context) async {
    ...
    try {
      ...
      final controller = await completer.future.timeout(const Duration(milliseconds: 1500));
  ...
  static Future<bool> printRaw(String text, BuildContext context) async {
    try {
      ...
      if (_controller != null && address != null) {
        _controller!.print(address: address);
        Fluttertoast.showToast(msg: 'Raw print sent', backgroundColor: Colors.green);
        return true;
      }
      ...
      Fluttertoast.showToast(msg: 'Raw print error: $e', backgroundColor: Colors.red);
      return false;
    }
  }

  /// Try printing a Receipt with retries. Returns true on success.
  static Future<bool> printWithRetry(Receipt receiptWidget, BuildContext context, {int retries = 2}) async {
    int attempt = 0;
...
class ReceiptBuilder {
  ...
  static Receipt milkReceipt(Map<String, dynamic> data) {
    return Receipt(
      builder: (context) {
        return Padding(
          padding: const EdgeInsets.only(top: 8.0, bottom: 36.0, left: 8.0, right: 8.0),
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                "Cumulative Weight: ${data['monthly_total'] ?? data['total']} L",
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  fontFamily: 'monospace',
                  fontSize: 28,
  ...
  static List<Widget> _wrapText(String text, {int width = 32, double fontSize = 18}) {
    ...
    return lines.map((l) => Text(l, style: TextStyle(fontFamily: 'monospace', fontSize: fontSize))).toList();
  }
