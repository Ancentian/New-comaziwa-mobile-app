    ...
    try {
      ...
      if (!contentType.toLowerCase().contains('application/json')) {
        ...
        await ErrorLogger.logNetworkError(
          ...
          rawBody: response.body,
          errorMessage: 'Received non-JSON response (expected application/json)',
          headers: response.headers,
        );
      ...
      if (data['status'] == 'success') {
        ...
        // -----------------------------
        // 🔥 SYNC FARMERS (After navigation)
        // -----------------------------
        Fluttertoast.showToast(msg: "Syncing farmers...");
        
        FarmerService().downloadFarmers().then((downloaded) {
          if (downloaded) {
            Fluttertoast.showToast(
