...
class _DashboardPageState extends State<DashboardPage> with SingleTickerProviderStateMixin {
  ...
  Future<String?> _getAuthToken() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString('auth_token') ?? prefs.getString('token') ?? prefs.getString('access_token');
  }
  ...
  DateTime _parseMonth(dynamic monthValue) {
    ...
    if (m != null && m >= 1 && m <= 12) return DateTime(DateTime.now().year, m, 1);
  ...
  Future<void> fetchDashboardData({DateTimeRange? range}) async {
    ...
    try {
      ...
      if (tenantId == null) {
        Fluttertoast.showToast(msg: "Missing tenant info. Please login again.", backgroundColor: Colors.redAccent);
        return;
      }
      ...
      url += query.contains("?") ? "&tenant_id=$tenantId" : "?tenant_id=$tenantId";
      ...
      if (res.statusCode == 200) {
        ...
        Fluttertoast.showToast(msg: "Failed to fetch data from server.", backgroundColor: Colors.redAccent);
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> _getUserProfile() async {
    ...
    try {
      final res = await http.get(Uri.parse("$apiBase/employee/profile"), headers: {
        'Accept': 'application/json',
  ...
  Future<void> _syncNow() async {
    ...
    if (success) {
      Fluttertoast.showToast(msg: "All data synced successfully", backgroundColor: Colors.green);
    } else {
      Fluttertoast.showToast(msg: "Some data could not be synced", backgroundColor: Colors.red);
    }
    _loadSyncStats();
  }
                  ...
                  Padding(
                    ...
                    child: Wrap(
                      ...
                      children: [
                        ...
                        _buildStatCard('Collections', '${dailyData.length}', Icons.add, Colors.orange, width: (screenWidth - 48) / 2),
                        _buildStatCard('Direct', '${(dailyData.length / 2).ceil()}', Icons.send, Colors.blue, width: (screenWidth - 48) / 2),
                        _buildStatCard('Farmers', '$totalFarmers', Icons.people, Colors.purple, width: (screenWidth - 48) / 2),
                        _buildStatCard('Unsynced', '$unsyncedCollections', Icons.sync, Colors.red, width: (screenWidth - 48) / 2),
                        GestureDetector(
                          onTap: _syncNow,
                          child: _buildStatCard('Sync Now', 'Tap to sync', Icons.cloud_upload, Colors.teal, width: (screenWidth - 48) / 2),
                        ),
                      ],
                    ),
                  ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.filter_alt_rounded, color: Colors.green, size: 28),
              ...
              child: Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _drawerTile(Icons.dashboard_outlined, 'Dashboard', '/dashboard'),
                    _drawerTile(Icons.local_drink_outlined, 'Milk Collection', '/milkCollection'),
                    _drawerTile(Icons.list_alt, 'Milk List', '/milkList'),
                    _drawerTile(Icons.people_outline, 'Members List', '/farmersList'),
                    const Divider(height: 1, thickness: 1),
                    _drawerTile(Icons.print_outlined, 'Printer Settings', null, isPrinter: true),
                    const Divider(height: 1, thickness: 1),
  ...
  ListTile _drawerTile(IconData icon, String title, String? route, {bool isPrinter = false, bool isLogout = false}) {
    return ListTile(
      ...
      onTap: () async {
        ...
        if (route == '/profile') {
          ...
          if (profile != null) {
            final displayName = profile['name'] ?? profile['full_name'] ?? profile['username'] ?? '';
          ...
          final prefs = await SharedPreferences.getInstance();
          final savedName = prefs.getString('name') ?? prefs.getString('full_name') ?? prefs.getString('user_name') ?? '';
                      ...
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          try {
                            ...
                            return Text(raw, style: const TextStyle(fontSize: 10));
                          }
                        },
                      ),
                    ),
                    ...
                    touchTooltipData: LineTouchTooltipData(
                      getTooltipItems: (touchedSpots) {
                        return touchedSpots.map((spotItem) {
                          ...
                          final label = (idx >= 0 && idx < monthLabels.length) ? monthLabels[idx] : '';
  ...
  Widget _buildMilkPieChart() {
    ...
    final totalMorning = monthlyData.fold<double>(
        0, (sum, item) => sum + ((item['total_morning'] ?? item['morning'] ?? 0) * 1.0));
    final totalEvening = monthlyData.fold<double>(
        0, (sum, item) => sum + ((item['total_evening'] ?? item['evening'] ?? 0) * 1.0));
